...
@app.route("/ngdemo")
def ngdemo():
    """ A route for a minimal working example of launching a neuroglancer viewer
    pointing to multiple cloudvolume container layers.
    This route will provide a link for the neuroglancer viewer hosting the cloudvolumes """
    ...
    while True:
        ...
        if 'viewer' in session_dict.keys():
            ...
            logging.debug("Still spinning; waiting for redis entry for neuorglancer viewer")
            time.sleep(0.25)
...
# @app.route("/make_precomputed")
# def make_precomputed():
#     """ A route for a minimal working example of making a precomputed 
#     dataset from a tif stack. """
#     logging.basicConfig(level=logging.DEBUG)
#     client = docker.DockerClient(base_url='unix://var/run/docker.sock')
